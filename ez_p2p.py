...
class p2pCommand(object):
  ...
  ping_request, ping_reply                               = range(9,11)
...
class client(threading.Thread):
  ...
  def receive(self, cmd = None):
    ...
    package_size = 4
    try:
      header_data, user_addr = self.receive_bytes(package_size)
      if len(header_data) == package_size:
        msg_len = struct.unpack('<L', header_data)[0]
        data, _ = self.receive_bytes(msg_len)
        if len(data) == msg_len:
          #self.replyQueue.put(self.success("received data"))
      ...
      for i in readable:
        if i == 0:
          ...
          if str(data[:-1]) == "close":
            ...
            #self.alive.clear()
            # takes too long and the readline is invoked again..
